...
export default function ModalAuthor({ open, toggle, items }) {
    ...
    const handleSubmit = (e) => {
        ...
        if (items !== "") {
            if (img) {
                axios.post("/files/upload", imgData).then((res) => {
                    if (res.status === 201) {
                        axios.patch(`/author/${items.id}`, { ...payload, image: res?.data?.link }).then((res) => {
                ...
                axios.patch(`/author/${items.id}`, { ...payload, image: items.image }).then((res) => {
                    window.location.reload();
                    toggle();
                })
            }
            ...
            axios.post("/files/upload", imgData).then((res) => {
                if (res.status === 201) {
                    axios.post("/author", { ...payload, image: res?.data?.link }).then((res) => {
                        window.location.reload();
                        toggle();
                    })
                }
